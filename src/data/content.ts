export const portfolioData = {
    hero: {
        name: "Atif Moin",
        role: "Software Engineer",
        tagline: "Innovative Frontend Engineer.",
        description: "Motivated and detail-oriented Software Engineer specializing in building responsive, user-centric web applications with React, Next.js, and modern UI libraries.",
        primaryAction: "View Projects",
        secondaryAction: "Contact Me"
    },
    skills: [
        { name: "React", level: 95, icon: "FaReact" },
        { name: "Next.js", level: 90, icon: "TbBrandNextjs" },
        { name: "TypeScript", level: 85, icon: "SiTypescript" },
        { name: "Redux Toolkit", level: 85, icon: "SiRedux" },
        { name: "Chakra UI", level: 90, icon: "SiChakraui" },
        { name: "Tailwind CSS", level: 90, icon: "SiTailwindcss" },
        { name: "Three.js", level: 75, icon: "SiThreedotjs" },
        { name: "Framer Motion", level: 80, icon: "SiFramer" },
        { name: "AngularJS", level: 70, icon: "FaAngular" },
        { name: "Flutter", level: 65, icon: "SiFlutter" },
        { name: "Node.js", level: 60, icon: "FaNodeJs" },
        { name: "Python", level: 50, icon: "FaPython" }
    ],
    experience: [
        {
            company: "Zopper",
            role: "Software Engineer",
            duration: "2023 - Present",
            description: "Built UI components for BOB Life Insurance, developed SaaS modules for Bank of Baroda, and improved performance on low-end devices. Implemented AES encryption and CSP policies."
        }
    ],
    projects: [
        {
            projectName: "Nested Comment and Reply",
            projectType: "Machine Coding Round Question Solution.",
            link: {
                code: "",
                project: "https://react-zgw1gz.stackblitz.io",
            },
            image: "https://img.freepik.com/free-vector/gradient-message-notification_23-2149179040.jpg?w=800"
        },
        {
            projectName: "Techicious",
            projectType: "A Freelancing Project (Work under Process.)",
            link: {
                code: "",
                project: "https://techicious.github.io/Techicious-Web/",
            },
            image: "https://img.freepik.com/free-vector/gradient-ui-ux-elements-background_23-2149056159.jpg?w=800"
        },
        {
            projectName: "Count ME",
            projectType: "Live Character Counter",
            link: {
                code: "",
                project: "https://atifmoin19.github.io/Live-char-counter/",
            },
            image: "https://img.freepik.com/free-vector/modern-check-mark-symbol-concept_23-2147984180.jpg?w=800"
        },
        {
            projectName: "Play Me",
            projectType: "A Music Player App",
            link: { code: "", project: "https://atifmoin19.github.io/MusicPlayer/" },
            image: "https://img.freepik.com/free-vector/gradient-music-festival-twitch-banner_23-2149026417.jpg?w=800"
        },
        {
            projectName: "CineFlow",
            projectType: "A customized movie tracking dashboard.",
            link: { code: "", project: "https://atifmoin19.github.io/netflix/" },
            image: "https://images.unsplash.com/photo-1574267432553-4b4628081c31?w=800&q=80"
        },
        {
            projectName: "RedStore",
            projectType: "An E-Commerce Website Design",
            link: {
                code: "",
                project:
                    "https://atifmoin19.github.io/E-commers-site/Mysite/indext.html",
            },
            image: "https://img.freepik.com/free-vector/ecommerce-web-page-concept-illustration_114360-8204.jpg?w=800"
        },
        {
            projectName: "GlassMorphism",
            projectType: "Component Based on glassmorphism.",
            link: {
                code: "",
                project: "https://atifmoin19.github.io/card-hover-animation/",
            },
            image: "https://img.freepik.com/free-vector/gradient-glassmorphism-background_23-2149447863.jpg?w=800"
        },
    ],
    contact: {
        email: "atifmoin19@gmail.com",
        socials: [
            { network: "GitHub", url: "https://github.com/Atifmoin19", icon: "FaGithub" },
            { network: "LinkedIn", url: "https://www.linkedin.com/in/atif-moin-858167179/", icon: "FaLinkedin" },
            { network: "Portfolio", url: "https://atifmoin19.github.io/three-portfolio/", icon: "FaGlobe" }
        ]
    }
};
